...
public class WeaponShopScreen extends JPanel {
    ...
    private void layoutComponentsForFullscreen() {
        if (exitBtn != null) {
            setGameBounds(exitBtn, 1360, 790, 150, 60);
            exitBtn.setFont(scaledFont("Microsoft JhengHei", Font.BOLD, 18));
            exitBtn.setText("返回陸地");
        }

        if (moneyLabel != null) {
            setGameBounds(moneyLabel, 1015, 120, 480, 40);
            moneyLabel.setFont(scaledFont("Microsoft JhengHei", Font.BOLD, 21));
        }

        if (scrollPane != null) {
            setGameBounds(scrollPane, 1000, 170, 500, 560);
        }

        if (weaponListPanel != null) {
            int cardW = Math.max(300, sw(460));
            int cardH = Math.max(120, sh(150));
    ...
    private JPanel createWeaponCard(Weapon weapon) {
        ...
        actionPanel.setPreferredSize(
            new Dimension(Math.max(78, sw(92)), Math.max(82, sh(96)))
        );
