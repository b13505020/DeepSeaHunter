...
public class Fish extends Item implements Serializable {
    ...
    public Fish(String name, double weight, int price, String imagePath, int maxHp, int rarityStars) {
        ...
        if (name.equals("螃蟹")) {
            this.y = 820;
            this.fishSize = 45;
        } else if (name.equals("鯊魚")) {
            this.y = random.nextInt(400) + 100;
            this.fishSize = 100;
        } else {
            this.y = random.nextInt(600) + 100;
        }
    ...
    public void updatePhysics(int px, int py) {
        ...
        if (pDist < 180 && pDist != 0) {
            ...
            if (!getName().equals("螃蟹")) {
                velY += (pdy / pDist) * scare;
            }
        }
        ...
        if (!getName().equals("螃蟹")) {
            y += velY + Math.sin(System.currentTimeMillis() * 0.003 + waveOffset) * 0.5;
        } else {
            y = 820;
        }
        ...
        if (!getName().equals("螃蟹") && (y < 0 || y > 850)) {
            velY *= -1;
            setNewTarget();
        }
    }
    ...
    public Rectangle getBounds() {
        return new Rectangle((int) x, (int) y, fishSize, fishSize);
    }
